...
(function () {
  ...
  C.Stage = function (p) {
    ...
    var measure = useCallback(function () {
      ...
      /* ⛔ **사다리는 위쪽을 촘촘히** 둡니다 (2026-08-30).
           예전 [1, 0.94, 0.88, 0.82 …] 은 칸이 6% 씩이라, **3px 넘쳤다고
           18% 를 깎는** 일이 생겼습니다 (재어 확인 — 계획 모음에서
           0.85 면 들어가는데 0.82 로 떨어졌습니다).
           깎이는 것은 그림·글자·단추 **모두**라, 태블릿처럼 낮은 화면에서는
           이 한 칸이 화면 전체를 작아 보이게 만듭니다.
         ▸ 위쪽 세 칸을 3% 로 잘게 나눕니다. 많이 줄여야 할 때는 어차피
           아래쪽으로 내려가므로 걸음 수가 늘어도 손해가 없습니다. */
      var FITS = [1, 0.97, 0.94, 0.91, 0.88, 0.85, 0.82, 0.76, 0.70, 0.64, 0.58];
      var fi = 0;
    ...
    /* ★ bare : **흰 상자를 벗깁니다** (2026-08-29 · 선생님 말씀 —
         「우선 큰상자를 없애자 … 너무 퍼져있음 시선분산 집중안되니
          가운데로 보기좋게 폭 줄이고 가운데 정렬해줘」).
       상자는 「여기까지가 한 덩어리」를 알려 주는 것인데, 화면 폭을 다 쓰는
  ...
  /* ⚠ 위 FITS 와 **똑같아야** 합니다 — indexOf 로 지금 칸을 찾기 때문입니다.
       하나만 고치면 못 찾아 늘 0 번째로 보고 한 칸만 줄입니다. */
  var GUARD_FITS = [1, 0.97, 0.94, 0.91, 0.88, 0.85, 0.82, 0.76, 0.70, 0.64, 0.58];
  ...
  C.useFitBox = function (ref) {
    useLayoutEffect(function () {
      function fit() {
        ...
        void el.offsetHeight;
        /* ⚠ 위 Stage 의 FITS 와 **똑같이** 촘촘하게 (2026-08-30). 여기만 성기면
             같은 화면인데 어느 장치가 재느냐에 따라 크기가 달라집니다. */
        var FITS = [1, 0.97, 0.94, 0.91, 0.88, 0.85, 0.82, 0.76, 0.70, 0.64, 0.58];
        var avail = el.clientHeight;
