...
(function () {
  ...
  C.PlanSheet = function (p) {
    ...
    /* ★ 1단계 계획표에는 `sheet-l1` 을 붙입니다 (2026-09-04 · 선생님 말씀 —
         「2단계는 노란 바탕의 문장 글자가 조금 크게 보였는데 1단계는 작게
          보인다 … 1단계에서도 크게 나올 수 있을 만큼은 크게」).
       까닭 : 1단계에는 위에 **그림 줄**이 한 줄 더 들어갑니다. 계획표 전체가
       남는 높이에 맞춰 한꺼번에 줄어드는데(C.GrowToFit), 그림 줄이 높이를
       먹은 만큼 **문장까지 함께** 작아졌습니다. 2단계는 그림 줄이 없어서
       같은 글자가 커 보였던 것입니다.
     ▸ 그래서 1단계에서만 문장 글자를 한 단 키우고, 좌우 여백을 줄여
       (14% → 10%) 늘어난 글자가 들어갈 자리를 냅니다. css 의 `.sheet-l1` 을
       보세요. */
    return html`<div class=${'sheet' + (pics.length ? ' sheet-l1' : '')}>
      ...
      ${cut
        ...
        ? (function () {
            ...
            /* ★ 칸 크기는 **개수에 따라** 줄입니다 (2026-09-01 첫 판에서
               ...
               ★★ **칸을 크게 키웠습니다** (2026-09-04 · 선생님 말씀 —
                 「오려붙이기에서 네모칸을 최대로 크게 할 수 있는 만큼 크게」).
                 20·24·28mm → **24·30·36mm**. A4 한 장(가로 186mm · 세로 273mm)
                 안에서 재어 보고 잡은 값입니다 — 다섯 칸이면
                 30×5 = 150mm 에 화살표와 사이가 28mm 라 186mm 에 들어갑니다.
               ⛔ 더 키우지 마세요 — 다섯 칸이 두 줄로 갈라지거나, 계획표와
                  학습지가 **한 장을 넘어갑니다**(선생님 : 「한 장으로」). */
            var sz = ps.length >= 6 ? '24mm' : (ps.length === 5 ? '30mm' : '36mm');
  ...
  C.PlanScreen = function (p) {
    ...
    var fresh = {
      /* ⚠ 값이 없을 때의 기본은 **쉬운 계획**입니다.
           store.js 의 새 학생 기본값(newStudent)과 **같아야** 합니다 —
           한쪽만 고치면 설정 화면과 실제 화면이 어긋납니다. */
    ...
    /* ★★ 「다음」은 **하위 활동 화면에서도 진짜로 넘어갑니다** (2026-09-04 ·
         선생님이 잡아 주심 — 「하위활동을 고르고 다음을 누르면 누구와
         할까요?가 나와야 하는데 안 나오고, 다른 활동 고르기를 눌러야
         누구와 할까요?로 가더라」).
       까닭은 이랬습니다 : `next()` 가 `step` 만 하나 올렸는데, 화면을 고르는
       `body()` 는 `if (subCard)` 를 **맨 앞에서** 봅니다. 그래서 step 은 이미
       `who` 로 갔는데 눈에는 하위 활동 화면이 그대로 남고, 나중에 `다른 활동
       고르기`로 subCard 를 비우는 순간 `누구와 할까요?` 가 튀어나온 것입니다.
       ▸ 넘어갈 때 subCard 를 **함께 비웁니다**. 그래야 적힌 대로 움직입니다 —
         「다음」은 다음 질문으로, 「다른 활동 고르기」는 앞의 `무엇을 할까요?` 로.
       ⛔ 여기서 subCard 비우는 줄을 지우지 마세요 — 같은 일이 그대로 돌아옵니다. */
    function next() {
      if (subCard) { subS[1](null); pageS[1](0); }
      if (step < KEYS.length - 1) setStep(step + 1);
    }
    ...
    return html`<div class="app" data-corner="plan">
      ...
      <!-- ★★★ **계획하기는 좌우 쪽으로 나누지 않습니다** (2026-09-04 · 선생님이
             교실 전자칠판에서 찍어 보내 주신 사진 — 5 / 8 「언제쯤 할까요?」
             화면에 고를 카드가 **한 장도 없고** 아래에 1 / 2 만 있었습니다).
           까닭 : 흰 칸은 내용이 넘치면 좌우 쪽(다단)으로 나눕니다. 그런데
           다단은 **줄이기(zoom)를 하기 전 높이**로 쪽을 가릅니다. 그래서 눈에는
           자리가 넉넉해 보이는데도 카드 묶음이 통째로 2쪽으로 밀려나고,
           1쪽에는 질문만 남아 **텅 빈 화면**이 됩니다. 이 고장은 이미
           common.js 의 onePage 주석에 적혀 있던 것입니다.
         ▸ 계획하기는 카드를 **제 손으로** 세 장씩 넘깁니다(arrowPager).
           흰 칸까지 또 나눌 까닭이 없습니다 — 넘치면 줄여서 한 쪽에 담습니다.
         ⛔ onePage 를 빼지 마세요. 빼면 전자칠판에서 빈 화면이 그대로 돌아옵니다.
         ⚠ 여가 일기도 같은 얼개입니다 — 거기서도 빈 화면이 보이면 같은 것을
           걸어 주세요 (지금은 완성 화면에만 걸려 있습니다).
         ⛔⛔ 이 주석 안에 **백틱 금지** — 백틱을 넣는 순간 html 템플릿이 거기서
             끊겨 화면이 통째로 죽습니다. 실제로 겪었습니다 (2026-09-04). -->
      <${C.Stage} top=${backBtn} action=${action} onePage=${true} tall=${true}>${body()}<//>
    </div>`;
  };
})();
