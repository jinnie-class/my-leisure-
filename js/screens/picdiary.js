...
(function () {
  ...
  /* 힌트 보기(그림칸에 뜨는 원고지)가 쓸 수 있는 속 크기.
     그림칸 속(770 × 513)에서 노란 상자의 테두리·여백과 `보고 써요` 딱지를 뺀 값입니다.
       ...
       세로 : 513 - 여백 14*2 - 테두리 3*2 - 딱지 30 - 사이 8*2 = 433
     ⛔ **사이 여백(gap 8)은 두 번 들어갑니다** — 딱지→제목, 제목→원고지.
     ⛔⛔ **딱지와 제목 줄 높이를 실제로 재어 고쳤습니다** (2026-08-30 ·
        선생님 : 「아래 힌트 내용이 짤려…」).
        재어 보니 딱지는 **29px**(26 으로 잡고 있었음), 제목 줄은 **51px**
        (48 로 잡고 있었음)이었습니다. 둘이 6px 모자라서 줄이 많은 일기에서
        마지막 줄이 잘렸습니다.
        ▸ 딱지 26 → **30**, 제목 48 → **56** 으로 넉넉하게 잡습니다.
     ⚠ 선생님이 「보고 써요 상자 때문이면 지우라」 하셨지만 **그 때문이
       아니었습니다.** 딱지는 어디에 쓰는 칸인지 알려 주는 표시라 그대로 둡니다.
     ⚠ css 의 `.pd-hintbig` 여백·테두리·gap 이나 `.pd-hintbig-lab` 크기를
       고치면 이 값도 같이 고쳐야 합니다. */
  var HINT_INNER_W = ART_INNER_W - 22 * 2 - 3 * 2;   // 720
  /* ⛔⛔ **노란 상자가 실제로 쓰는 높이는 513 이 아니라 487 입니다**
       (2026-08-30 재어 확인 · 선생님 : 「아래 힌트 내용이 짤려…」).
       셈으로는 그림칸 높이가 770 / 1.501 = 513 인데,
         .pd-draw 실제 높이        509px
         그 안의 .pd-hintbig 높이  487px  ← 상자가 쓰는 것은 이만큼
       26px 을 **더 있다고 보고** 셈해서, 줄이 많은 일기에서 마지막 줄이
       잘렸습니다 (8줄짜리에서 20px 잘림 — 재어 확인).
     ⚠ 「보고 써요」 딱지 때문이 아니었습니다. 딱지는 어디에 쓰는 칸인지
       알려 주는 표시라 그대로 둡니다 (선생님께 지우지 말라 여쭙고 확인). */
  var HINT_BOX_H = Math.round(ART_INNER_W / ART_RATIO) - 26;   // 487
  var HINT_INNER_H = HINT_BOX_H - 14 * 2 - 3 * 2 - 30 - 8 * 2;  // 407
  /* 힌트 안의 **제목 줄**이 쓰는 높이 (2026-08-28 · 선생님 말씀 — 「제목 힌트가 없어」).
     ⚠ 재어 보니 **51px** 입니다 (2026-08-30). 48 로 잡고 있어 모자랐습니다.
       넉넉히 56 으로 둡니다 — 제목이 길어 글자가 커지는 날도 있습니다.
