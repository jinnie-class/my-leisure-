...
(function () {
  ...
  /* 힌트 보기(그림칸에 뜨는 원고지)가 쓸 수 있는 속 크기.
     그림칸 속(770 × 513)에서 노란 상자의 테두리·여백과 `보고 써요` 딱지를 뺀 값입니다.
       ...
       세로 : 513 - 여백 14*2 - 테두리 3*2 - 딱지 26 - 사이 8*2 = 437
     ⛔ **사이 여백(gap 8)은 두 번 들어갑니다** — 딱지→제목, 제목→원고지
        (2026-08-30 · 선생님 : 「보고 써요가 잘림」).
        한 번만 빼서 8px 이 모자랐고, 상자가 `justify-content:center` 라
        모자란 만큼이 **위아래로 나뉘어 잘려**, 맨 위 딱지가 반쯤 사라졌습니다.
      ▸ 함께 css 의 `.pd-hintbig` 를 위쪽 붙임(flex-start)으로 바꿨습니다.
        셈이 조금 어긋나도 **딱지부터 잘리는 일은 없습니다.**
     ⚠ css 의 `.pd-hintbig` 여백·테두리·gap 을 고치면 이 값도 같이 고쳐야 합니다. */
  var HINT_INNER_W = ART_INNER_W - 22 * 2 - 3 * 2;   // 720
  var HINT_INNER_H = Math.round(ART_INNER_W / ART_RATIO) - 14 * 2 - 3 * 2 - 26 - 8 * 2;  // 437
