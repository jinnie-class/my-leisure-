...
(function () {
  ...
  /* 기분 문장 : "신나고 뿌듯했어요."
     ★★ **「기분이」를 뺐습니다** (2026-09-04 · 선생님 말씀 —
         「기분이 신나요. 기분이 재미있어요 … 기분이와 다 맞는 걸까?」).
       까닭 : 「기분이 ○○하다」의 ○○ 자리에는 **기분 자체의 상태**를
       나타내는 말만 들어갑니다 (편안하다 · 뿌듯하다 · 좋다). 그런데
         · 신나다 · 화나다 는 **사람**이 주어이고 (신난 건 나)
         · 재미있다 는 **활동**이 주어이며 (재미있는 건 줄넘기)
         · 힘들다 는 **일이나 몸**이 주어입니다
       주어가 다른 말을 한 틀에 밀어 넣어서 어색해졌습니다.
       특히 「화났어요」는 우리말로 「**화가** 났어요」라, 「기분이 화났어요」는
       주어를 두 번 세운 꼴이 됩니다.
     ▸ 「기분이」를 빼면 아홉 가지가 **모두** 자연스러워집니다 —
       「좋았어요.」 「신나고 재미있었어요.」 「힘들었어요.」
     ▸ 묻는 말(「기분이 어땠나요?」)은 그대로 둡니다 — 그쪽은 자연스럽고,
       무엇을 답하는 칸인지 알려 주는 말이기 때문입니다.
     ⛔ 「기분이」를 도로 붙이지 마세요. 넷은 맞고 넷은 어긋납니다. */
  S.diaryMood = function (d) {
    var ids = (d && d.moodIds) || [];
    var ms = ids.map(App.mood).filter(Boolean);
    if (!ms.length) return '';
    if (ms.length === 1) return ms[0].past + '.';
    var head = ms.slice(0, -1).map(function (m) { return m.conn; });
    return head.join(' ') + ' ' + ms[ms.length - 1].past + '.';
  };
  ...
  /* 활동 이름을 문장에 쓰기 좋게 줄인 형태 : '박물관 가기' → '박물관'
     ⛔⛔ **띄어쓰기가 있는 것만** 떼어 냅니다 (2026-08-24).
       예전에는 `보기` · `가기` 처럼 **붙어 있는 것도** 뗐습니다. 그래서
       「식물 돌보기」가 **「식물 돌」** 이 되었습니다 — 「돌보기」는 한 낱말인데
       뒤 두 글자를 잘라 버린 것입니다. 「돌보기 · 알아보기 · 돌아보기」처럼
       `보기` 로 끝나는 한 낱말이 많아 이런 일이 자꾸 납니다.
     ▸ 「캠핑하기 → 캠핑」 만은 붙여 써도 안전해서 남깁니다. 우리말에서
       `-하기` 는 앞말에 붙는 씨끝이라 그 앞이 늘 온전한 낱말입니다.
     ⛔ 목록에 붙여 쓴 꼴(`'보기'` · `'가기'`)을 다시 넣지 마세요.
     ⛔ 잘라 낸 뒤 **띄어쓰기로 끝나면** 안 자릅니다 — 「그림 그리기」가
       「그림」 으로 줄어드는 것은 괜찮지만, 낱말 가운데를 자르면 안 됩니다. */
  var TRIM = ['하기', ' 가기', ' 보기', ' 키우기', ' 모으기', ' 그리기', ' 만들기'];
