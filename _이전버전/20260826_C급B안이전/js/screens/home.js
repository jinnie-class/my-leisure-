...
(function () {
  ...
  C.HomeScreen = function (p) {
    ...
    var switchS = useState(false);     // 학생 바꾸기 팝업 (홈에서 바로 바꿉니다)
    ...
    return html`<div class="app" data-corner="home">
      <${C.TopBar}
        ...
        <!-- ★ 이름표를 누르면 **그 자리에서** 다른 학생으로 바꿉니다 (2026-08-25).
               예전에는 학생 고르기 화면으로 나갔는데, 화면을 떠나지 않고
               바꾸는 쪽이 빠릅니다. 학생을 더하거나 고치는 일은
               팝업의 「학생 화면으로 가기」 로 갑니다. -->
        <${C.WhoChip} student=${student} onClick=${function () { switchS[1](true); }}
      ...
      ${switchS[0] && html`<${C.Modal} title="누가 할까요?" onClose=${function () { switchS[1](false); }}
        ...
        actions=${html`<${React.Fragment}>
          <${C.Btn} onClick=${function () { switchS[1](false); p.nav('profiles'); }}>학생 화면으로 가기<//>
          <${C.Btn} onClick=${function () { switchS[1](false); }}>닫기<//>
        <//>`}>
        <!-- .big : 팝업 안 그림을 작게 만드는 .modal .pick .thumb 규칙(970줄)을 이깁니다 -->
        <div class="pick-grid cols-3 big">
          ${App.store.get().students.map(function (s) {
            var cur = student && s.id === student.id;
            return html`<button key=${s.id} type="button" class=${'pick' + (cur ? ' sel' : '')}
                ...
                onClick=${function () {
                  ...
                  switchS[1](false);
                  App.ui.toast(App.josa(s.name, '으로/로') + ' 바꿨어요.');
